...
impl Solver {
    fn new(side_len: usize, dictionary: Dict, print_sols: bool) -> Self {
        Solver { side_len, dictionary , print_sols }
    }
    ...
    fn are_cols_valid(&self, grid: &Grid, grid_info: &GridInfo) -> bool {
        for col_index in 0..self.side_len {
            ...
            if !self.dictionary.does_infix_exist(&col_str, grid_info.starting_row) {
                return false;
            }
        ...
        if num_rows == 1 {
            ...
            .for_each(|top_word_chunk| {
                ...
                .for_each(|bottom_word_chunk| {
                    ...
                    if self.print_sols && grid_clone.len() == self.side_len {
    ...
    fn solve(&self) -> Vec<Grid> {
        ...
        let res = self.divide_and_conquer(&initial_grid_info);
        println!("\nDone!");
        return res;
    }
}

pub fn run_solver(side_len: usize, dict_path: &str, print_sols: bool) -> Vec<Grid>{
    // TODO: can I avoid repeated transpose solutions?
    let solver = Solver::new(
        side_len,
        Dict::new(dict_path.to_string(), side_len),
        print_sols,
    );
